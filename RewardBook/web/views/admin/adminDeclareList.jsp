...
<script type="text/javascript">
	$(function() {
		...
		$(".main_area2_section2 .search .submit").on("click", function() {
			var link = "page=declare&movePage=1";
			...
			location.href="/RewardBook/dc_list_a?" + link;
		});
		...
		$(".main_area3_section1 .pageButton#page1").on("click", function() {
			var link = "page=declare&movePage=<%= currentPage - 2 %>";
			link += "&s1check1=" + $(".main_area2_section1 .checkbox span input[name=s1check1]").is(":checked");
			link += "&s1check2=" + $(".main_area2_section1 .checkbox span input[name=s1check2]").is(":checked");
			link += "&s2select1=" + $(".main_area2_section2 .search select[name=s2select1]").val();
			link += "&s2text1=" + $(".main_area2_section2 .search input[name=s2text1]").val();
		...
		$(".main_area3_section1 .pageButton#page2").on("click", function() {
			var link = "page=declare&movePage=<%= currentPage - 1 %>";
			link += "&s1check1=" + $(".main_area2_section1 .checkbox span input[name=s1check1]").is(":checked");
			link += "&s1check2=" + $(".main_area2_section1 .checkbox span input[name=s1check2]").is(":checked");
			link += "&s2select1=" + $(".main_area2_section2 .search select[name=s2select1]").val();
			link += "&s2text1=" + $(".main_area2_section2 .search input[name=s2text1]").val();
		...
		$(".main_area3_section1 .pageButton#page6").on("click", function() {
			var link = "page=declare&movePage=<%= maxPage %>";
			link += "&s1check1=" + $(".main_area2_section1 .checkbox span input[name=s1check1]").is(":checked");
			link += "&s1check2=" + $(".main_area2_section1 .checkbox span input[name=s1check2]").is(":checked");
			link += "&s2select1=" + $(".main_area2_section2 .search select[name=s2select1]").val();
			link += "&s2text1=" + $(".main_area2_section2 .search input[name=s2text1]").val();
		...
		<div class="main_area2">
			...
			<input type="hidden" name="page" value="project">
				<div class="main_area2_section1">
					<div class="checkbox">
						<span><input type="checkbox" name="s1check1" <% if (s1check1.equals("true")) { %>checked="checked"<% } %>> 확인</span>
						<span><input type="checkbox" name="s1check2" <% if (s1check2.equals("true")) { %>checked="checked"<% } %>> 미확인</span>
					</div>
				</div>
				...
				<div class="table">
					<table cellspacing="0" cellpadding='0' style="width : 1175px; table-layout: fixed;">
						...
						<% for (Inquiry inquiry : list) { %>
							<tr OnClick="location.href='/RewardBook/iq_detail_a?page=Inquiry&iq_no=<%= inquiry.getIq_no() %>'" style="cursor : pointer;"><td><%= inquiry.getIq_no() %></td><td><%= inquiry.getU_name() %></td><td><%= inquiry.getIq_title().length() > 20 ? inquiry.getIq_title().substring(0, 20) + "..." : inquiry.getIq_title() %></td><td><%= inquiry.getIq_date() %></td><td><%= inquiry.getIq_oc() %></td><td><%= inquiry.getA_name() %></td><td><%= inquiry.getIq_rdate() %></td></tr>
						<% } %>
